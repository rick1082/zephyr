...
static struct bt_audio_stream streams[1];
...
static void print_codec(const struct bt_codec *codec)
{
	...
	if (codec->id == BT_CODEC_LC3_ID) {
		/* LC3 uses the generic LTV format - other codecs might do as
		 * well */
		...
		if (bt_codec_cfg_get_chan_allocation_val(
			    codec, &chan_allocation) == 0) {
			printk("  Channel allocation: 0x%x\n", chan_allocation);
		}

		printk("  Octets per frame: %d (negative means value not "
		       "pressent)\n",
...
lc3_config(struct bt_conn *conn, struct bt_audio_ep *ep, enum bt_audio_dir dir,
...
static struct bt_audio_capability caps = {
	...
	.pref = BT_AUDIO_CAPABILITY_PREF(BT_AUDIO_CAPABILITY_UNFRAMED_SUPPORTED,
					 BT_GAP_LE_PHY_2M, 0x02, 10, 40000,
					 40000, 40000, 40000),
	.codec = &lc3_codec,
	.ops = &lc3_ops,
};
...
void main(void)
{
	...
	bt_audio_capability_register(&caps);
